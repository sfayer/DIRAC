...
class AccountingDB(DB):
    ...
    def deleteType(self, typeName):
        ...
        tablesToDelete = []
        tablesToDelete.append(self._getTableName("in", typeName))
        tablesToDelete.append(self._getTableName("bucket", typeName))
        tablesToDelete.append(self._getTableName("type", typeName))
        for keyField in self.dbCatalog[typeName]["keys"]:
            tablesToDelete.append(self._getTableName("key", typeName, keyField))
        req = "DROP TABLE "
        req += ",".join(tablesToDelete)
        retVal = self._query(req)
        if not retVal["OK"]:
            return retVal
        req = "DELETE FROM "
        req += self._getTableName("catalog", "Types")
        req += " WHERE `name`=%s"
        retVal = self._update(req, args=(typeName,))
        if not retVal["OK"]:
            return retVal
    ...
    def deleteRecord(self, typeName, startTime, endTime, valuesList):
        ...
        sqlCond = []
        condArgs = []
        mainTable = self._getTableName("type", typeName)
        ...
        for i, value in enumerate(sqlValues):
            ...
            if needToRound:
                ...
                sqlCond.append(f"`{mainTable}`.`{self.dbCatalog[typeName]['typeFields'][i]}`=%s")
                condArgs.append(value)
        retVal = self._getConnection()
        if not retVal["OK"]:
            return retVal
        ...
        req += " WHERE "
        req += " AND ".join(sqlCond)
        retVal = self._update(req, args=condArgs, conn=connObj)
        if not retVal["OK"]:
            return retVal
